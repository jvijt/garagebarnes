...
/**
 * Remove OceanWP-related extras on normal blog posts where possible.
 */
add_filter('ocean_display_related_posts', '__return_false', 99);
add_filter('ocean_display_post_next_prev', '__return_false', 99);

/**
 * Build the Garage Barnes news layout inside the post content.
 */
add_filter('the_content', function ($content) {
    ...
    if ($first_paragraph === '') {
        $first_paragraph = '<p>' . esc_html(wp_trim_words(wp_strip_all_tags($content), 45, '…')) . '</p>';
    }
    ...
    <article class="gb-news-detail">
        ...
        <?php if ($image) : ?>
            <div class="gb-news-detail-intro">
                ...
                <div class="gb-news-detail-lead"><?php echo wp_kses_post($first_paragraph); ?></div>
            </div>
        <?php else : ?>
            <div class="gb-news-detail-lead gb-news-detail-lead-full"><?php echo wp_kses_post($first_paragraph); ?></div>
        <?php endif; ?>

        <div class="gb-news-detail-body">
            <?php echo wp_kses_post($remaining); ?>
        </div>
        ...
        <?php if ($prev || $next) : ?>
            <nav class="gb-news-detail-nav" aria-label="Vorige en volgende berichten">
                <div class="gb-news-nav-side gb-news-nav-prev">
                    <?php if ($prev) : ?>
                        <a href="<?php echo esc_url(get_permalink($prev)); ?>"><span>← Vorig bericht</span><strong><?php echo esc_html(get_the_title($prev)); ?></strong></a>
                    <?php endif; ?>
                </div>
                <div class="gb-news-nav-side gb-news-nav-next">
                    <?php if ($next) : ?>
                        <a href="<?php echo esc_url(get_permalink($next)); ?>"><span>Volgend bericht →</span><strong><?php echo esc_html(get_the_title($next)); ?></strong></a>
                    <?php endif; ?>
                </div>
            </nav>
        <?php endif; ?>
...
/**
 * Detail page styling, closely aligned with the vehicle detail styling.
 */
add_action('wp_head', function () {
    ...
    <style id="gb-news-detail-css">
      ...
      .gb-news-detail-header h1{margin:0 0 12px;font-size:clamp(38px,5vw,66px);line-height:1.02;letter-spacing:-.04em;color:#202020}
      .gb-news-detail-date{color:#7b7b7b;font-size:14px}

      .gb-news-detail-intro{display:grid;grid-template-columns:minmax(0,1.35fr) minmax(300px,.85fr);gap:30px;align-items:stretch;margin-bottom:34px}
      ...
      .gb-news-detail-lead{display:flex;align-items:center;padding:38px;border:1px solid #e1e4de;border-radius:12px;background:#fff;font-size:22px;line-height:1.65}
      .gb-news-detail-lead p{margin:0;color:#444}
      .gb-news-detail-lead-full{display:block;max-width:850px;margin-bottom:34px}
      ...
      .gb-news-detail-body h2{margin:46px 0 16px;font-size:34px}
      .gb-news-detail-body h3{margin:36px 0 14px;font-size:26px}
      ...
      .gb-news-nav-side a:hover{border-color:#5dc01d;transform:translateY(-2px)}
      .gb-news-nav-side span{color:#5dc01d;font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:.08em}
      .gb-news-nav-side strong{font-size:18px;line-height:1.3}
      .gb-news-nav-next{text-align:right}

      @media(max-width:800px){
        .gb-news-detail{padding:44px 18px 70px}
        .gb-news-detail-intro{grid-template-columns:1fr;gap:18px}
        .gb-news-detail-media,.gb-news-detail-image{min-height:0}
        .gb-news-detail-image{aspect-ratio:4/3}
        .gb-news-detail-lead{padding:26px;font-size:19px}
        .gb-news-detail-body{font-size:17px}
        .gb-news-detail-nav{grid-template-columns:1fr}
