...
function gb_over_ons_team_shortcode(){
    ...
    $team = array(
        array('name'=>'Peter Barnes','roles'=>array('Oprichter','Eigenaar','Takelaar','Onthaal'),'phone'=>'+32 492 06 15 45','phone_href'=>'+32492061545','email'=>'peter@garagebarnes.com'),
        array('name'=>'David Barnes','roles'=>array('Administratie','Boekhouding'),'email'=>'info@garagebarnes.com'),
        array('name'=>'Melvin Barnes','roles'=>array('Onthaal','Administratie'),'email'=>'info@garagebarnes.com'),
    ...
    <main class="gb-page gb-about-page">
      ...
      <?php foreach ($team as $member): ?>
        <article class="gb-team-card"><div class="gb-team-photo"><img src="<?php echo esc_attr($placeholder); ?>" alt="Tijdelijke foto van <?php echo esc_attr($member['name']); ?>"></div><div class="gb-team-card-body"><h3><?php echo esc_html($member['name']); ?></h3><div class="gb-team-role-list"><?php foreach($member['roles'] as $role): ?><span><?php echo esc_html($role); ?></span><?php endforeach; ?></div><?php if (!empty($member['phone']) || !empty($member['email'])): ?><div class="gb-team-contact"><?php if (!empty($member['phone'])): ?><a href="tel:<?php echo esc_attr($member['phone_href']); ?>"><?php echo esc_html($member['phone']); ?></a><?php endif; ?><?php if (!empty($member['email'])): ?><a href="mailto:<?php echo esc_attr($member['email']); ?>"><?php echo esc_html($member['email']); ?></a><?php endif; ?></div><?php endif; ?></div></article>
      <?php endforeach; ?>
      </div></div></section>
